    ...
    for i, line in enumerate(f):
        if section in line:
            ...
            try:
                ...
                pass
            if len(found) != len(stringhe):
                raise SystemExit(
                    "\nERROR!\nSection '%s' of file '%s' does not contain string(s): %s."
                    % (section, f.name, ', '.join([s for s in stringhe if s not in found]))
                )
            return i + n, [found[s] for s in stringhe]
    raise SystemExit(
